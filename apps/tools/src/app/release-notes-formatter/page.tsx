...
import ReleaseNotesFormatterContent from "./content";
import { TOOL_FAQ } from "@/data/tool-faq";

export const generateMetadata = () => generateToolMetadata("/release-notes-formatter");

const TOOL_HREF = "/release-notes-formatter";
const tool = getToolByHref(TOOL_HREF)!;
const collection = getPrimaryCollection(tool);

export default function ReleaseNotesFormatterPage() {
  ...
  return (
    <>
      ...
      <ToolPageWrapper href={TOOL_HREF}>
        <ReleaseNotesFormatterContent faqPassages={faq} />
      </ToolPageWrapper>
    </>
  );
}
