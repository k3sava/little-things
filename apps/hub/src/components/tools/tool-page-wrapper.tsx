...
function ShortcutHintBar() {
  ...
  return (
    <div
      className="kami-shortcut-bar fixed bottom-0 left-0 right-0 z-40 flex items-center justify-center gap-4 px-4 py-2 text-xs transition-opacity duration-300"
      style={{ opacity: visible ? 0.7 : 0, pointerEvents: visible ? "auto" : "none" }}
    >
      {labeled.map((s, i) => (
        <span key={i} className="flex items-center gap-1">
          <kbd className="inline-block rounded px-1.5 py-0.5 font-mono text-[10px]">
